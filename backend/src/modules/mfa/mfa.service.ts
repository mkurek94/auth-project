...
export class MfaService {
  ...
  public async verifyMFASetup(req: Request, code: string, secretKey: string) {
    ...
    if (user.userPreferences.enabled2FA) {
      return {
        message: "MFA already enabled.",
        userPreferences: {
          enable2FA: user.userPreferences.enabled2FA,
        },
      };
    }
    ...
    user.userPreferences.enabled2FA = true;
    await user.save();
  ...
  public async revokeMFA(req: Request) {
    ...
    user.userPreferences.enabled2FA = false;
    user.save();
  ...
  public async verifyMFAForLogin(code: string, email: string, userAgent?: string) {
    ...
    if(!user.userPreferences.enabled2FA && !user.userPreferences.twoFactorSecret) {
